...
def get_top_retweets():
    ...
    if not cache.get(query):
        ...
        top_retweet_ids_raw = Session.query(colname).from_statement(text(query)).all()
...
def get_wordcloud():
    ...
    if not cache.get(query):
        logging.info(f"Cache MISS: {query}")
        tweets = Session.query('tweet_text').from_statement(text(query)).all()
        Session.commit()
...
def latest_tweet_stream(n):
    while True:
        try:
            query = query_last_n(Tweet.__tablename__, n, track_term=YANG_TERM)

            latest_tweet_objects = Session.query(Tweet).from_statement(
                text(query))
...
def tweets_chart_request(chart_type, track_term=YANG_TERM):
    count_colname = 'count'
